...
import {
  ...
  TablePagination, MenuItem, Select, FormControl, InputLabel, TableHead, TableRow,
  TableCell, Checkbox, TextField, Dialog, DialogTitle, DialogContent, DialogActions,
  IconButton, Tooltip
} from '@mui/material';
      ...
      setUsers((prev) =>
        prev.map((user) =>
          user.id === selectedUser.id
            ? { ...user, isVerified: selectedUser.isVerified, status: selectedUser.status }
            : user
        )
      );
      setOpenDialog(false);
                ...
                <TableRow>
                  ...
                  <TableCell>
                    <FormControl size="small" fullWidth>
                      ...
                      <Select value={filterRole} label="Role" onChange={handleFilterRole}>
                  ...
                  <TableCell align="center">
                    <FormControl size="small" fullWidth>
                      ...
                      <Select value={filterStatus} label="Status" onChange={handleFilterStatus}>
                        <MenuItem value="All">All</MenuItem>
              ...
              <TableBody>
                {visibleRows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      <Tooltip title={row.isVerified ? 'Verified' : 'Not Verified'} arrow>
                        <Checkbox
                    ...
                    <TableCell>
                      <Tooltip title="Edit User" arrow>
                        <IconButton onClick={() => handleOpenDialog(row)} aria-label="edit">
                          <EditIcon />
                        </IconButton>
                      </Tooltip>
                    </TableCell>
                ...
                {emptyRows > 0 && <TableEmptyRows height={77} emptyRows={emptyRows} />}
                {filteredUsers.length === 0 && <TableNoData query={`${filterRole} + ${filterStatus}`} />}
              </TableBody>
            </Table>
          </TableContainer>
        </Scrollbar>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          <Stack spacing={2}>
            ...
            <TextField label="Phone" value={selectedUser?.phone || ''} fullWidth disabled />
            ...
            <FormControl>
              <Stack direction="row" alignItems="center" spacing={1} sx={{ width: '50%' }}>
                <Checkbox
                  ...
                  onChange={(e) => handleDialogChange('isVerified', e.target.checked)}
                />
